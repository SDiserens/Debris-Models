	...
	double *posn, double *vel)
{
	...
	if (vel)
	{
		...
		const double vel_at_perihelion =    /* in AU/day */
			sqrt( muGravity * (1. + elem->ecc) / elem->q);
...
ELEMENTS Construct_elem(OrbitalElements &elements)
{
	ELEMENTS elem;
	elem.ecc = elements.eccentricity;
	elem.q = (1. - elem.ecc) * elements.semiMajorAxis;
	elem.incl = RadtoDeg(elements.inclination); 
	elem.asc_node = RadtoDeg(elements.rightAscension);
	elem.arg_per = RadtoDeg(elements.argPerigee);
	/* l = (100.46435 + (129597740.63 / 3600.) * t_cen) * PI / 180.; */
	derive_quantities(&elem, muGravity);
...
{
	ELEMENTS elemP = Construct_elem(primary);
	ELEMENTS elemS = Construct_elem(secondary);

	double distance = find_moid_full(&elemP, &elemS, mdata);
