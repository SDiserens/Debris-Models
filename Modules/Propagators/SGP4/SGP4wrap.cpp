...
void SGP4::UpdateElements(DebrisObject &object, double timeStep)
{
	...
	if (!object.SGP4Initialised())
	{
		...
		double objectEpoch = max(object.GetInitEpoch(), population.GetStartEpoch());
		objectEpoch += 2834.0000; //Corrected to 0 Jan 1950 (i.e. 31/12/49) for SGP4
		// call SGP4init
	...
	// Recent DeActivation
		...
		double objectEpoch = object.GetEpoch() + 2834.0000; //Corrected to 0 Jan 1950 (i.e. 31/12/49) for SGP4
																// call SGP4init
		SGP4Funcs::sgp4init(gravModel, opsMode, object.GetID(), objectEpoch, bStar, 0, 0, //first and second derivative of the mean motion set to zero as unused
			elements.eccentricity, elements.argPerigee, elements.inclination, elements.GetMeanAnomaly(), meanMotionKozai, elements.rightAscension,
			object.GetSGP4SatRec());
	...
	timeMinutes = max(object.GetInitEpoch(), population.GetStartEpoch());
	timeMinutes = (population.GetEpoch() - timeMinutes) * 24 * 60;
		// call SGP4 procedure
